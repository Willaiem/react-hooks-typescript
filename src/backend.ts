...
type PokemonGraphqlVariables = {
  name: string
}

const pokemonApi = graphql.link('https://graphql-pokemon2.vercel.app/')

const isAllPokemonKey = (name: string): name is keyof typeof allPokemon => {
  return name.toLowerCase() in allPokemon
}

export const handlers = [
  pokemonApi.query<Record<string, unknown>, PokemonGraphqlVariables>('PokemonInfo', (req, res, ctx) => {
    const key = req.variables.name
    if (isAllPokemonKey(key)) {
